...
def generate_secs():
    ...
    arr1 = []
    for i in range(30):
        arr.append(random.uniform(0.5,1.5))
    sum = 0
    for i in arr:
        arr1.append(i)
        arr1.append(2-i)
    ...
    #random.shuffle(arr1)
    print(arr1)
    shuffled_list = sorted(arr1, key=lambda x: random.random())
    #print(sum)
    return shuffled_list

def display_numbers():
    for i in range(1,13):
        x = round(-68*math.sin(-30*(i)*0.01745329))
        y = round(-68*math.cos(-30*(i)*0.01745329))
        
        printstring("{:X}".format(i),x+115,y+115,2,True,False,colour(255,255,255))
    


for r in range(10):
    ring(120,120,90+r,colour(255,255,0))
for r in range(5):
    ring(120,120,1+r,colour(255,255,0))
display_numbers()
LCD.show()
...
while(running):
    secondl = generate_secs()
    print(secondl)
    for h in range(60):
        xh = round(-40*math.sin(-6*h*0.01745329))
        yh = round(-40*math.cos(-6*h*0.01745329))
        LCD.line(120,120,xh+120,yh+120,colour(0,255,0))
        LCD.show()
        for m in range(60):
            xm = round(-50*math.sin(-6*m*0.01745329))
            ym = round(-50*math.cos(-6*m*0.01745329))
            ...
            for s in range(60):
                for r in range(5):
                    ring(120,120,1+r,colour(255,255,0))
                x = round(-60*math.sin(-6*s*0.01745329))
